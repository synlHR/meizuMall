...
var isMyEmailUrl = '/uc/webjsp/member/isMyEmailCheck';
...
$(function () {
    ...
    if (isBindPhone) {
        $('#changePwdValidPhone').show();
        $('#changePhoneValidPhone').show();
    } else {
        $('#changePwdValidEmail').show();
        $('#changePhoneValidEmail').show();
    }
    $('.show_valid_wrap').click(function () {
        $(this).parents('.modify_content').find('.change_valid_wrap').show();
        $(this).parents('.change_valid_wrap').hide();
        // $('#changePwdValidEmail').show();
        // $('#changePwdValidPhone').hide();
    });
});
...
$.extend(Manage.prototype, {
    ...
    getAllOptions: function () {
        ...
        var data = [
...

            {
                ...
                successcallback: function (i, result) {
                    ...
                    $(data[i].content).find("input").val("");
                    ReceiveEmail = self.hideEmail(text);
                    self.removePwdCss();
            ...
            },
            {
                ...
                successcallback: function (i, result) {
                    ...
                    $(data[i].content).find("input").val("");
                    ReceiveEmail = self.hideEmail(text);  //默认加***展示
                    //self.getSaveLevel();
                    self.removePwdCss();
                    self.resetResize(848);
                    location.reload();
            ...
            },
            {
                ...
                url: checkValidPasswordUrl + '?type=email_update',
                eventcallback: function (i) {
                    self.addPwdCss();
                },
                successcallback: function (i) {
            ...
            },
            {
                ...
                modify: "",
                title: "#telWrap",
                content: "#bindTel-step2",
                nextContent: "#bindTelWrap",
                save: "#ce-u-phone-email-check-bind",
                cancel: "#ce-u-phone-email-cancel-bind",
                url: checkOldPhoneUrl + '?type=phone&vCodeTypeValue=26',
                eventcallback: function () { },
                successcallback: function (i, result) {
                    $(data[i].content).find("input").val("");
                    self.resetResize(1100);
                ...
                errorcallback: function (message, errorCode, fn) {
                    ...
                    if (errorCode == showErrorVCode) {
                        util.addTips("kapkeyTel-check-email-bind", message);
                        return;
                    }
                    fn && fn();
                },
            ...
            },
            {
                ...
                url: "/uc/webjsp/member/bindPhone?vCodeTypeValue=6&v=2",
                eventcallback: function () { },
                successcallback: function (i, result) {
                    var item = $("#telModify");
                    item.siblings(".item-middle").hide();
            ...
            },
            {
                modify: "#editMobile",
                title: "#telWrap",
                content: "#updateTel-step1",
                nextContent: "#setTelCheckPass",
                save: "#updateTel-step1-setTelCheckPassSave",
                cancel: "#updateTel-step1-setTelCheckPassCancel",
                url: checkValidPasswordUrl + '?type=phone_update_psw',
                eventcallback: function () { },
                successcallback: function (i, result) {
                    $(data[i].content).find("input").val("");
                    self.resetResize(1100);
                ...
                errorcallback: function (message, errorCode) {
                    ...
                    if (errorCode == showPasswordErrorCode) {
                        util.addTips("ce-u-passwordNew-updateTel", message);
                    }
                },
                rules: util.createRule({
                    password: {
        ...
        var email = {
            modify: "#modifyPassword",
            title: "#pwdWrap",
            formId: '#formPwdValidEmail',
            content: "#changePasswordWrapFirst",
            nextContent: "#changePasswordWrap",
            save: "#ce-u-pwd-email-check",
            cancel: "#ce-u-pwd-email-cancel",
            url: checkOldPhoneUrl + '?type=password&vCodeTypeValue=26',
            eventcallback: function () { },
            successcallback: function (i, result) {
                $(data[i].content).find("input").val("");
                self.resetResize(1100);
            ...
            errorcallback: function (message, errorCode, fn) {
                ...
                if (errorCode == showErrorVCode) {
                    util.addTips("kapkeyTel-check-email", message);
                    return;
                }
                fn && fn();
            },
        ...
        var changePhoneValidPhone = {
            modify: "",
            title: "#telWrap",
            formId: '#formPhoneValidPhone',
        ...
        var changePhoneValidEmail = {
            modify: "",
            title: "#telWrap",
            formId: '#formPhoneValidEmail',
            content: "#setTelCheckPass",
            nextContent: "#changeTelWrap-activeNew",
            save: "#ce-u-phone-email-check",
            cancel: "#ce-u-phone-email-cancel",
            url: checkOldPhoneUrl + '?type=phone_update&vCodeTypeValue=26',
            eventcallback: function () { },
            successcallback: function (i, result) {
                $(data[i].content).find("input").val("");
                self.resetResize(1100);
            ...
            errorcallback: function (message, errorCode, fn) {
                ...
                if (errorCode == showErrorVCode) {
                    util.addTips("change-phone-kapkeyTel-check-email", message);
                    return;
                }
                fn && fn();
            },
        ...
        if (isBindPhone && isBindEmail) {
            ...
            data.push(phone);
            data.push(email);
            data.push(changePhoneValidPhone);
            data.push(changePhoneValidEmail);
        } else if (isBindPhone) {
    ...
    getKapkey: function () {
        ...
        $("form").delegate(".get_kapkey", "click", function (e) {
            ...
            var vCodeType = 12;
            var param = {};
            if (targetId == 'getKeyonePassword') {
                targetId = 'getKeyone';
            }
            if (targetId == 'getKeyonePhone') {
                targetId = 'getKeyone';
            }
            ...
            var bindEmail = form.find("input[name=bindEmail]");
            ...
            if (targetId == "getKeyone" && code != 26) { //邮箱有省略拿全局变量里完整的
            ...
            if (!!param.phone) {
                ...
                setTimeout(function () {
                    ...
                    if ($("#ce-u-password_email").is(":visible") && $("#ce-u-password_email").hasClass("error")) {
                        $("#getKeyonePassword").addClass("invalidBtn");
                        return;
                    }
                    if ($("#ce-u-bindphone_email").is(":visible") && $("#ce-u-bindphone_email").hasClass("error")) {
                        $("#getKeyoneBindPhone").addClass("invalidBtn");
                        return;
                    }

                    if ($("#ce-u-phone_email").is(":visible") && $("#ce-u-phone_email").hasClass("error")) {
                        $("#getKeyonePhone").addClass("invalidBtn");
                        return;
                    }
    ...
    saveEvent: function (json, index) {
        ...
        form.validate($.extend(util.simpleValidate, {
            ...
            submitHandler: function () {
                // if (json.save === '#updateTel-step1-setTelCheckPassSave' && !isBindEmail && !isBindPhone) {
                //     nAlert('请绑定邮箱', '系统提示');
                //     return;
                // }
                var form = $(json.content).find('form:visible');
                var data = CountryCode.getFormData(form);
                if (data['phone']) {
                    data['phone'] = '00' + ($('#cycode').html() - 0) + ':' + data['phone']
                }
                if (json.save === "#ce-u-pwd-email-check" || json.save === "#ce-u-phone-email-check" || json.save === '#ce-u-phone-email-check-bind') {
                    data.kapkey = data.kapkeyEmail;

                    data.email = data.bindEmail;
                }
                if (json.save === "#ce-u-pwd-phone-check" || json.save === "#setTelCheckPassSave") {
                    data.kapkey = data.kapkeyPhone;
                    if (data.email || data.email == '') {
                        delete data.email;
                    }
                }
                if (data.kapkeyEmail || data.kapkeyEmail == '') {
                    delete data.kapkeyEmail;
                }
                if (data.kapkeyPhone || data.kapkeyPhone == '') {
                    delete data.kapkeyPhone;
                }
                if (data.bindEmail || data.bindEmail == '') {
                    delete data.bindEmail;
                }
            ...
            showErrors: function (errorMap, errorList) {
                ...
                if (phoneMsg) {
                    ...
                    if (!$('#err-bind-phone-tips').length)
                        $('<span id="err-bind-phone-tips" for="phone" class="error">' + phoneMsg + '</span>').insertAfter($cycodeBox)
        ...
        $(json.save).click(function () {
            ...
            var len = form.length;
            if (json.save === '#ce-u-pwd-phone-check' || json.save === "#setTelCheckPassSave") {
                var form = $(json.content).find('form').eq(0);
            } else if (json.save === '#ce-u-pwd-email-check' || json.save === '#ce-u-phone-email-check') {
                if (len > 1) {

                    var form = $(json.content).find('form').eq(1);
                }
            }
            form.submit();
        });
    ...
    mailAuto: function () {
        ...
        mzMailAuto('ce-u-password_email');
        mzMailAuto('ce-u-bindphone_email');
        mzMailAuto('ce-u-phone_email');
    },
    resize: function () {
        var $con = $('#content'),
            _this = this;
...
function bindTip() {
    if (isHit == "true") {
        $("#show-msg-pub").addClass("tip-pub-show");
        var $showTip = $("#show-psw-tip");
        $("#header-top").addClass("tip-header-overflow-show");
        $showTip.addClass("tip-bg");
        $showTip.on('click', function (e) {
            $showTip.addClass("tip-content-hide");
            $("#show-msg-pub").removeClass("tip-pub-show");
            $("#header-top").removeClass("tip-header-overflow-show");
            util.doAsyncPost("/uc/webjsp/account/updateHitCheckStatus", function (result) { }, {});
        });
    }
}
